...
### BUG-P1: CAPITAL_MAX_BOT=300 vs saldo real ~165 USDC
**Ficheiro:** `config.py` linha 47
**Problema:** `CAPITAL_MAX_BOT = 300.0` mas saldo real é ~165 USDC. O bot usa o mínimo entre saldo real e CAPITAL_MAX_BOT para sizing — portanto não é crítico (165 < 300, usa 165). Mas o comentário em `MAX_MARGEM_TRADE` diz "60 USDC em 300" que está errado.
**Impacto:** Baixo — sizing usa saldo real via `get_balance()`. Mas confunde ao ler o código.
**Fix:** Actualizar `CAPITAL_MAX_BOT = 165.0` ou deixar como cap de segurança e corrigir o comentário.

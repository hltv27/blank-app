...
export default function AddSubscriptionModal({ onClose, onSave, existing }: Props) {
  ...
  const handleQuickFill = (svc: typeof POPULAR_SERVICES[0]) => {
    setName(svc.name); setEmoji(svc.emoji); setColor(svc.color); setCategory(svc.category as Category);
  };
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
      ...
      <div className="card modal-in" style={{ width: '100%', maxWidth: 480, padding: 28, maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h2 style={{ fontSize: 18, fontWeight: 700 }}>{existing ? t('subscriptions.edit_title') : t('subscriptions.new_title')}</h2>
        ...
        {!existing && (
          <div style={{ marginBottom: 20 }}>
            <label>{t('subscriptions.popular')}</label>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
              {POPULAR_SERVICES.slice(0, 12).map(svc => (
                <button key={svc.name} onClick={() => handleQuickFill(svc)} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, padding: '5px 10px', cursor: 'pointer', color: 'var(--text)', fontSize: 12, display: 'flex', alignItems: 'center', gap: 4, transition: 'all 0.15s' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr', gap: 10 }}>
            <div>
              <label>{t('subscriptions.icon')}</label>
              <input value={emoji} onChange={e => setEmoji(e.target.value)} maxLength={2} style={{ textAlign: 'center', fontSize: 22, padding: '8px 4px' }} />
            </div>
            <div>
              <label>{t('subscriptions.name_label')}</label>
              <input value={name} onChange={e => setName(e.target.value)} placeholder={t('subscriptions.name_placeholder')} required />
            </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div>
              <label>{t('subscriptions.amount_label')}</label>
              <input type="number" step="0.01" min="0" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0.00" required />
            </div>
            <div>
              <label>{t('subscriptions.cycle_label')}</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div>
              <label>{t('subscriptions.next_billing')}</label>
              <input type="date" value={nextBillingDate} onChange={e => setNextBillingDate(e.target.value)} />
            </div>
            <div>
              <label>{t('subscriptions.category_label')}</label>
          ...
          <div>
            <label>{t('subscriptions.color_label')}</label>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 4 }}>
              {COLORS.map(c => (
                <button key={c} type="button" onClick={() => setColor(c)} style={{ width: 28, height: 28, borderRadius: '50%', background: c, border: 'none', cursor: 'pointer', outline: color === c ? '3px solid white' : 'none', outlineOffset: 2, transition: 'outline 0.15s' }} />
              ))}
            </div>
          </div>

          <div>
            <label>{t('subscriptions.payment_label')}</label>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button type="button" onClick={() => setActive(!active)} style={{ width: 42, height: 24, borderRadius: 12, background: active ? 'var(--accent)' : 'var(--surface2)', border: 'none', cursor: 'pointer', position: 'relative', transition: 'background 0.2s' }}>
              <div style={{ width: 18, height: 18, borderRadius: '50%', background: 'white', position: 'absolute', top: 3, left: active ? 21 : 3, transition: 'left 0.2s' }} />
            </button>
            <span style={{ fontSize: 13, color: 'var(--muted)' }}>
              {active ? t('subscriptions.is_active') : t('subscriptions.is_paused')}
            </span>
          </div>

          <div style={{ display: 'flex', gap: 10, paddingTop: 4 }}>
            <button type="button" onClick={onClose} className="btn-ghost" style={{ flex: 1 }}>{t('common.cancel')}</button>
            <button type="submit" className="btn-primary" style={{ flex: 1, justifyContent: 'center' }}>
              {existing ? t('common.save') : t('subscriptions.add')}
            </button>
          </div>
        </form>
      </div>
